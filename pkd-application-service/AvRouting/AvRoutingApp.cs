...
internal class AvRoutingApp : IAvRoutingApp, IDisposable
{
    ...
    public ReadOnlyCollection<AvSourceInfoContainer> GetAllAvSources()
    {
        ...
        {
            var avr = _switchers.FirstOrDefault(x => x.Id == source.Matrix);
            var supportsInputSync = avr as IVideoInputSyncDevice;
            container.Add(new AvSourceInfoContainer(
                ...
                source.Control)
            {
                SupportSync = supportsInputSync != null,
                HasSync = supportsInputSync?.QueryVideoInputSyncState((uint)source.Input) ?? false
            });
        }
    ...
    public void MakeRoute(string inputId, string outputId)
    {
        ...
        {
            Logger.Error(e,
                $"pkd-application-service.AvRoutingApp.MakeRoute() - cannot make route for {inputId} -> {outputId}");
        }
    }
    ...
    /// <inheritdoc/>
    public bool QueryVideoInputSyncStatus(string id)
    {
        var source = _sources.FirstOrDefault(x => x.Id.Equals(id));
        if (source == null)
        {
            Logger.Error($"AvRoutingApp.QueryVideoInputSyncStatus() - no source with id {id}");
            return false;
        }

        var avr = _switchers.FirstOrDefault(x => x.Id.Equals(source.Matrix));
        return avr is IVideoInputSyncDevice videoSyncAvr &&
               videoSyncAvr.QueryVideoInputSyncState((uint)source.Input);
    }
    ...
    private void SubscribeDevices()
    {
        foreach (var switcher in _switchers)
        {
            ...
            if (switcher is IVideoInputSyncDevice supportsVideoInputSync)
            {
                supportsVideoInputSync.VideoInputSyncStateChanged += SwitcherInputSyncChanged;
            }
        }
    }
    ...
    private void SwitcherRouteChanged(object? sender, GenericDualEventArgs<string, uint> e)
    {
        ...
        var dest = _destinations.Find(x =>
            x.Output == e.Arg2 && x.Matrix.Equals(e.Arg1, StringComparison.InvariantCulture));
        if (switcher == null || dest == null) return;

        var newSrc = _sources.FirstOrDefault(x => x.Input == switcher.GetCurrentVideoSource(e.Arg2) &&
    ...
    private void SwitcherInputSyncChanged(object? sender, GenericSingleEventArgs<uint> args)
    {
        ...
        var source = _sources.FirstOrDefault(x =>
            x.Input == args.Arg &&
            x.Matrix.Equals(baseDevice.Id));

        if (source == null) return;
        var temp = VideoInputSyncChanged;
        temp?.Invoke(this, new GenericSingleEventArgs<string>(source.Id));
    }
    ...
    private void SendRouteToDevices(List<Vertex> path)
    {
        ...
        foreach (var node in path)
        {
            ...
            if (nodeData[1].Equals("IN", StringComparison.InvariantCulture))
            {
                ...
                {
                    Logger.Error(e, "AvRoutingApp.SentRouteToDevice() - unable to parse input {0} for source {1}",
                        nodeData[2], nodeData[1]);
                    return;
                }
            }
            else if (nodeData[1].Equals("OUT", StringComparison.InvariantCulture))
            {
                ...
                {
                    Logger.Error(e, "AvRoutingApp.SendRouteToDevice() - Unable to parse output {0} for destination {1}",
